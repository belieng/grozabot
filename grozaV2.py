...
# В aiogram 3.x используется HTML вместо ParseMode.HTML в строках, 
# но для простоты оставим передачу строкой
...
# ИСПОЛЬЗУЕМ ":" КАК РАЗДЕЛИТЕЛЬ ДЛЯ БЕЗОПАСНОСТИ ID
...
async def process_admin_action(callback: CallbackQuery):
    ...
    try:
        ...
        _, action, user_id, chat_id, msg_id = callback.data.split(":")
...
async def monitor(message: Message):
    ...
    if PATTERN.search(message.text):
        ...
        # Кнопки с двоеточием в качестве разделителя
        kb = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🚫 ЗАБАНИТЬ", callback_data=f"adm:ban:{user_id}:{message.chat.id}:{message.message_id}")],
            [InlineKeyboardButton(text="➕ В исключения", callback_data=f"adm:whitelist:{user_id}:{message.chat.id}:{message.message_id}")]
        ])
...
async def main():
    print("⚡️ ГРОЗА запущена и готова к очистке...")
